<template>
  <div class="hello">
    ...
    <div class="span4">
        ...
        <input id="radius" v-model="radius" type="range" min="1" max="50" step="1" value="5"/>
        <label>blur size</label>
        <input id="blur" v-model="blur" type="range" min="1" max="50" step="1" value="15"/>
        </form>
    </div>
  </div>
</template>
    ...
    mapInit(){
      ...
      this.map = new Map(
        ...
        var vector = new Heatmap({
            ...
            blur: parseInt(this.radius, 10), //模糊大小，以像素为单位
            radius: parseInt(this.blur, 10), //半径大小，以像素为单位
        });
